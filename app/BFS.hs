...
isWalkable' env pos isWalkableCondition = let element = getElementAtPosition env pos in
    case element of
        Nothing -> False
        Just element -> isWalkableCondition element
...
getValidNeighbors (r, c) grid isWalkableCondition v = catMaybes [right', down', left', up'] -- >>> sortBy (\(a,_) -> putDirtFirst (env) (a))
  where
    (rowMax, colMax) = (rowSize grid, colSize grid)
    right = (r, c + 1)
