...
randomlyPutDirt env emptiesElements maxToPut = let randomEmptyElement = pickRandom emptiesElements 
    in let updatedEnvironment =  env {dirts = deleteFromList (Dirt (pos randomEmptyElement)) (dirts env) ++ [Dirt (pos randomEmptyElement)] ,empties = deleteFromList (EmptyCell(pos randomEmptyElement)) (empties env) }
...
generateInSquare environment position =
    let emptiesInSquare = filter (inSquare position) ((empties environment))
            in let nKids = length (filter (inSquare position) ((children environment)))
